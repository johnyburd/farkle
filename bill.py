...
def calculated_expected(curr: int, num: int, iterations=0):
    if num in calculated and curr in calculated[num]:
        pass
...
def analyze_roll(rolls, total = 0):
    ...
    for score, remaining in options:
        ...
        print(f"{score:<6} {','.join(str(c) for c in remaining):<12} {round(expected + score, 2):<10} {round(expected + score + total, 2):<10}")
